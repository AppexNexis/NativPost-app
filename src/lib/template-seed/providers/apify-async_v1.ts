...
export async function startTikTokIngest(params: StartParams) {
  ...
  const runId = await startActorRun(
    TT_ACTOR_ID,
    {
      ...
      // Ask Apify to host the video + cover in its key-value store so the
      // dataset items include downloadable videoUrl and covers.default —
      // without these flags, the mapper stores empty media_url/thumbnail_url.
