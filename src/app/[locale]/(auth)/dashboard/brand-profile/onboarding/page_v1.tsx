...
function StepBusinessBasics({ data, onChange, errors }: StepProps) {
  ...
  const handleImport = async () => {
    ...
    if (!url) {
      setImportError('Enter a website URL first.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setImportError(json.error || 'Could not read that site. Fill this in manually instead.');
        return;
      }
      ...
      // something the user already typed before clicking import.
      const updates: Partial<BrandProfileData> = {};
      if (p.brandName && !data.brandName.trim()) {
        updates.brandName = p.brandName;
      }
      if (p.industry && !data.industry.trim()) {
        updates.industry = p.industry;
      }
      if (p.targetAudience && !data.targetAudience.trim()) {
        updates.targetAudience = p.targetAudience;
      }
      if (p.companyDescription && !data.companyDescription.trim()) {
        updates.companyDescription = p.companyDescription;
      }
      if (typeof p.toneFormality === 'number') {
        updates.toneFormality = p.toneFormality;
      }
      if (typeof p.toneHumor === 'number') {
        updates.toneHumor = p.toneHumor;
      }
      if (typeof p.toneEnergy === 'number') {
        updates.toneEnergy = p.toneEnergy;
      }
      if (Array.isArray(p.vocabulary) && p.vocabulary.length > 0 && data.vocabulary.length === 0) {
        updates.vocabulary = p.vocabulary;
      }
      if (p.communicationStyle && !data.communicationStyle.trim()) {
        updates.communicationStyle = p.communicationStyle;
      }
      if (Array.isArray(p.contentExamples) && p.contentExamples.length > 0 && data.contentExamples.length === 0) {
        updates.contentExamples = p.contentExamples;
      }
      ...
      onChange(updates);
      setImportedFields(json.fieldsFound || []);
    } catch {
      setImportError('Something went wrong reaching the import service. Try again, or fill this in manually.');
    } finally {
    ...
    <div className="space-y-5">
      <div className="rounded-lg border border-dashed border-primary/30 bg-primary/5 p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Paste your site and we'll pre-fill what we can find. You'll still review and edit everything below before saving.
        </p>
        <div className="mt-3 flex gap-2">
          ...
          <button
            ...
            disabled={isImporting}
            className="inline-flex shrink-0 items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors disabled:opacity-60"
          >
            {isImporting
              ? <Loader2 className="size-4 animate-spin" />
              : <Sparkles className="size-4" />}
            {isImporting ? 'Reading site...' : 'Import'}
          </button>
        </div>
        {importError && (
          <p className="mt-2 text-xs text-destructive">{importError}</p>
        )}
        {importedFields && importedFields.length > 0 && (
          <p className="mt-2 flex items-center gap-1 text-xs text-emerald-600">
            ...
            {' '}
            field
            {importedFields.length === 1 ? '' : 's'}
            {' '}
            below from your site. Review before continuing.
          </p>
        )}
        {importedFields && importedFields.length === 0 && (
          <p className="mt-2 text-xs text-muted-foreground">
            Couldn't find much usable copy on that page. Fill in the fields below manually.
          </p>
        )}
      </div>
