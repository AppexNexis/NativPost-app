...
export async function GET(request: NextRequest) {
  ...
  try {
    // Step 1 — Pull all graphic_urls for this org from the DB
    const rows = await db
      .select({ graphicUrls: contentItemSchema.graphicUrls })
      .from(contentItemSchema)
      .where(eq(contentItemSchema.orgId, orgId!));

    // Step 2 — Extract unique UUIDs from all graphicUrls arrays
    ...
    if (uuidSet.size === 0) {
      return NextResponse.json({ assets: [], total: 0, nextCursor: null });
    }

    const allUuids = [...uuidSet];

    // Step 3 — Fetch file metadata from Uploadcare for each UUID.
    // Uploadcare doesn't have a batch-by-UUIDs endpoint, so we fetch
    // the file list filtered to stored files, then cross-reference UUIDs.
    // For orgs with many files we paginate internally.
    const PAGE_SIZE = 100;
    let fetchedAssets: MediaAsset[] = [];
    let nextFrom: string | null = null;
    let fetched = 0;

    // Fetch pages until we have all our UUIDs covered
    while (fetched < allUuids.length) {
      ...
      for (const file of results) {
        if (uuidSet.has(String(file.uuid))) {
    ...
    // Step 5 — Sort newest first (Uploadcare already sorts but this ensures it
    // after our UUID cross-reference reorders things)
    ...
    return NextResponse.json({
      ...
      total,
      nextOffset,
      // Keep nextCursor null — we use offset-based pagination now
      nextCursor: null,
    });
  } catch (err) {
