...
import { Loader2, Music, Volume2, X } from 'lucide-react';
...
function AudioSelectModal({
  ...
  const fetchAudio = useCallback(async () => {
    ...
    setError(null);
    try {
      // Try audio first, fall back to video assets
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          {!loading && !error && assets.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground/60">Audio tracks will appear here once they are scraped and imported.</p>
            </div>
          )}
          <div className="space-y-2">
            {assets.map(a => (
              ...
              >
                ...
                <button
                  onClick={() => onSelect({ name: a.name, url: a.url, publicId: a.publicId, source: 'library' })}
