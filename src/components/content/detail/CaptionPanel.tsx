    ...
    <Card className="p-4 sm:p-5">
      ...
      {isEditing
        ...
        : caption
          ...
          : <p className="text-body italic text-muted-foreground">No caption yet.</p>}
      ...
      {hashtags.length > 0 && (
        <div className="mt-4 border-t pt-3">
          <div className="mb-2 flex items-center justify-between gap-2">
            ...
            <Button size="sm" variant="ghost" className="h-6 gap-1 px-1.5 text-micro" onClick={copyHashtags}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {hashtags.map(tag => (
              <span key={tag} className="rounded bg-muted px-1.5 py-0.5 text-meta text-muted-foreground">{tag}</span>
            ))}
          </div>
        </div>
      )}
