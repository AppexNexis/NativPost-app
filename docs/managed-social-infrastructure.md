...
**Status:** activation stack is built (see below). Managed Posting (priority 1) has a live workflow;
the rest are catalogued + priced (`status: 'planned'`) until their workflow ships.

**Activation (built).** `addon-service.ts` (`listOrgAddons`/`getOrgAddon`/`isAddonActive`/
`activateAddon`/`deactivateAddon`, upsert on the (org, addon) unique index) + pure
`validateActivation`/`requiresTier`/`resolveTier` in `addons.ts`. API `GET/POST /api/msi/addons`
(catalog + org state; activate/deactivate). Dashboard `/dashboard/infrastructure/addons` — catalog
cards with who-performs badge, tier picker, activate/deactivate; "Request early access" for planned.

**Managed Posting (built — the first real add-on workflow).** Pure `managed-posting.ts`
(`MANAGED_POSTING_QUOTA` {starter 12, professional 30, scale 60}, `quotaForTier`, `remainingPosts`,
`canRequestPost`, `buildContentPostJob`). New job type `content_post` (draft_content → peer_review
→ publish), SLA 24h. Service `managed-posting-service.ts`: `getPostingStatus` (active/tier/quota/
used/remaining), `countPostsThisPeriod` (content_post jobs this UTC month = quota), `requestManagedPost`
(gates on active + quota + account ownership → inserts a draft `content_item` + a `content_post`
job/tasks). API `GET/POST /api/msi/addons/managed-posting` (status; quota-gated request). UI
`/dashboard/infrastructure/addons/managed-posting` (quota bar + request form). **Reuse:** the
content_post job appears on the existing Ops queue for operators; on **QA approval**
(operations-service qa branch) it calls `enqueueManagedPublish` for the linked content → the
existing publish pipeline (worker → adapter → platform) → billable event. Nothing new in the
publish/worker/billing path.

**Operator drafting (built).** `content-draft-service.ts` (`getContentPostDraft`/
`saveContentPostDraft` on the linked content_item), admin API `GET/POST
/api/admin/msi/jobs/[jobId]/draft`, and a `ContentPostDraft` panel on the admin account job board
(`/admin/msi/[id]`) shown for content_post jobs: reads the brief, edits caption + graphic + type,
saves. Approval/publish stay in `JobActions`. Closes the loop: customer requests → operator drafts
→ review → QA → publish.

**Add-on billing (built, gated).** `addon-billing.ts` mirrors the metered seam: gated on
`MSI_ADDON_BILLING_ENABLED`, prices from env `STRIPE_ADDON_PRICE_<ADDON>[_<TIER>]`, lazy Stripe
client. `activateAddon` best-effort attaches a Stripe subscription item to the org's subscription
(re-prices on tier change) and stores `stripeSubscriptionItemId`; `deactivateAddon` removes it.
Safe no-op until the flag + price env vars are set — activation never depends on Stripe. **To go
live:** create a Stripe price per fixed-tier add-on tier, set `STRIPE_ADDON_PRICE_*` env vars, flip
`MSI_ADDON_BILLING_ENABLED=true`. +21 pure tests; full suite green.

---

## 18. Changelog

- **2026-07-27 (Kick the worker on managed publish — near-instant).** The publish route now fires a **best-effort** POST to `/api/cron/msi-worker` (Bearer `CRON_SECRET`, fire-and-forget) when a managed account was targeted, so the worker starts immediately instead of waiting for the 5-minute cron. Sync platforms (LinkedIn/Facebook/YouTube) publish in that first tick; async Instagram *starts* immediately and confirms on the next tick. The scheduled cron remains the guaranteed fallback — a dropped kick only means slightly slower, never lost. Needs `NEXT_PUBLIC_APP_URL` + `CRON_SECRET` set. (To further cut async-IG latency, raise the `msi-worker.yml` cron frequency.) type-clean.
- **2026-07-27 (Publishing is the approval — no manual review queue for publishes).** Removed the pointless post-hoc approval queue for managed publishes. A `publish_post` job now **auto-completes** (in_progress → completed via a new `autoApproved` state-machine guard), firing the billable event + marking "Published to" published — no peer_review/qa. Rationale: a **customer-authored** managed post (created in the GUI, managed account selected) needs no operator approval — the customer authorized it by publishing, and it should go out like the OAuth accounts beside it. For **operator-drafted** content (Managed Posting add-on), the human review already gates the publish *upstream* on the `content_post` draft (draft → review → approve → **then** enqueue the publish) — so the resulting `publish_post` must not re-review. `autoCompletePublishJob` helper handles both the async (confirm pass) and sync (start pass) publish paths. Provisioning jobs (`create_account`, etc.) still route through peer_review → qa. +1 test; suite **376/376**, type-clean.
- **2026-07-27 (Fix: official_api publishes stuck in `queued`).** A managed publish to an `official_api` account (e.g. an Instagram publish_post) hung in `queued` forever because the worker's allocation step tried to assign an **operator + device** to every queued job — but API-executed accounts have (and need) none, so `planAllocations` could never assign it, and the job never reached `in_progress` → `peer_review` (which is where the Approve action appears). This was the "it's Queued but there's nothing to approve, then it shows up hours later" symptom. Fix: allocation is now **strategy-aware** — `official_api` queued jobs auto-assign (`queued → assigned` via a new `apiExecuted` guard on the state machine; no operator/device) so the start pass runs them through the adapter on the next tick; only `manual`/`delegated_access` jobs go through capacity allocation. (The old workaround — seeding jobs as `assigned` — is no longer needed.) +1 test; suite **375/375**, type-clean.
- **2026-07-27 (Fix: Managed Posting → "Published to" visibility).** Closed the gap where a `content_post` publish didn't surface in the customer's "Published to" panel. On QA approval, the content_post hook now inserts a `queued` `publishing_queue` row (socialAccountId = the managed account's linked `social_account`, matching what the worker's `syncManagedPublicationRow` keys on) before enqueuing the publish — so the worker flips it to `published` + permalink on completion, exactly like an OAuth or route-driven managed publish. Best-effort (never fails QA); skipped when the account has no linked social_account. type-clean, suite **374/374**.
- **2026-07-27 (Catalog complete — all 10 add-ons live).** Flipped the last four to `available` with value content: **Expansion** (per-account; workflow link → the Configure Accounts flow), **Influencer** + **Localization** (custom/quote), **Recovery** (per-case). Custom/per-case add-ons are **quote requests**, not self-serve: the in-app card shows "Request a quote" / "Requested", and operators see them in a new **Service requests — follow up with a quote** section on `/admin/msi/addon-ops` (query `listServiceRequests` for active influencer/localization/recovery subs + `GET /api/admin/msi/service-requests`). Web `/managed` now lists all 10 (added Influencer + Localization to the marketing catalog, "Custom quote" pricing with a scope note). type-clean, suite **374/374**. **Every catalog entry now has a real activation path: 6 self-serve workflows (Posting, Content, Analytics, Advertising, Community, UGC), Expansion via the core order flow, and 3 quote-request services (Influencer, Localization, Recovery).**
- **2026-07-27 (Managed UGC add-on + operator delivery surfaces).** Sixth live add-on + completed the operator loop. **UGC (per-deliverable):** new `ugc_video` job type (produce → review, SLA 72h), pure `managed-ugc.ts` ($25/video, `buildUgcJob`), `managed-ugc-service.ts` (`requestUgcVideo` → draft content_item + ugc_video job). On QA approval (operations-service) the content is delivered (status approved) **and** a $25 fee is billed via `billOneOffInvoiceItem`. Customer API/UI (`/managed-ugc`). Operators draft via the shared `ContentPostDraft` (now content_post | content_piece | ugc_video). **Operator surfaces:** `/admin/msi/addon-ops` gained an **Analytics reports to deliver** section (list in_review → Deliver button → existing deliver endpoint), alongside the ad-spend + community-log tools. New admin query `listReportsForReview` + `GET /api/admin/msi/analytics-reports`. **Also fixed** a pre-existing repo bug: `notify.ts` hard-coded `category: 'system'` while its own test expected `'infrastructure'` (the notification's action URL is an infrastructure link) — aligned the impl to the test. +3 tests; suite **374/374**, type-clean. **Six live add-ons: Posting, Content, Analytics, Advertising, Community, UGC.**
- **2026-07-26 (Managed Advertising + Managed Community — non-flat billing).** Two more live add-ons, and the first non-flat billing model. **Ads (percent-of-spend):** `msi_ad_campaign` table (migration `0056`), pure `managed-ads.ts` ($49 setup fee, 10–20% pct validation, `computeManagementFeeCents`), `managed-ads-service.ts` (`createCampaign` bills a one-time $49 setup invoice item; `recordSpend` [operator] bills a management-fee invoice item = pct × spend). New billing primitive `billOneOffInvoiceItem` in addon-billing.ts (`stripe.invoiceItems.create` on the org's customer — for fees that don't fit a flat subscription price). Customer API/UI (`/managed-ads`, create + list, pct slider) + operator spend API `/api/admin/msi/ad-campaigns/[id]/spend`. **Community (flat-tier):** `msi_community_reply` table (migration `0056`), pure `managed-community.ts` (quota basic 500 / active 1000 / unlimited -1), `managed-community-service.ts` (status/quota, `logReplies` [operator]), customer status API/UI + operator log API `/api/admin/msi/community/log`. Both flipped to `available`. +9 tests; suite **371/371**, type-clean. **Five add-ons now live: Posting, Content, Analytics, Advertising, Community.**
- **2026-07-26 (Managed Content add-on — third live add-on).** Reuses the Managed Posting spine, distinct deliverable: new `content_piece` job type (draft → review, NO publish; SLA 24h), pure `managed-content.ts` (quota lite 15 / growth 40 / studio 80; `buildContentPieceJob`; reuses generic `canRequestPost`/`remainingPosts`), `managed-content-service.ts` (status/quota/`requestContentPiece` → draft content_item + content_piece job). Customer API `GET/POST /api/msi/addons/managed-content` + UI `/dashboard/infrastructure/addons/managed-content`. **Key distinction from Posting:** on QA approval a content_piece marks the linked content_item `approved` (delivered to the customer's library to schedule) — NOT auto-published (operations-service). Operator drafting reuses `ContentPostDraft` (now shown for both content_post + content_piece; content-draft-service accepts both). +5 tests; suite **362/362**, type-clean. Three add-ons now live: Posting, Content, Analytics.
- **2026-07-26 (Managed Analytics add-on + add-on billing verify script).** Second real add-on. `msi_analytics_report` table (migration `0055`, one report per account/month), pure `managed-analytics.ts` (`composeReportSummary` adapts copy to the numbers, `reportPeriodOf`, `canDeliver` lifecycle), `managed-analytics-service.ts` (`getReports`, `requestReport` — gated on the add-on, idempotent per account/month, post counts from the billable-event ledger, `deliverReport`). Customer API `GET/POST /api/msi/addons/managed-analytics` + UI `/dashboard/infrastructure/addons/managed-analytics` (generate + view delivered reports). Operator deliver API `POST /api/admin/msi/analytics-reports/[id]/deliver`. Add-ons card now links per-add-on workflows via `ADDON_WORKFLOWS`. Plus `scripts/msi-verify-addon-billing.ts` (`msi:verify-addon-billing`) — Stripe-only readiness check for every fixed-tier add-on price (env var set? price exists? amount matches catalog?). +6 tests; suite **357/357**, type-clean.
- **2026-07-26 (MSI Add-ons: operator drafting + add-on Stripe billing).** Closed the Managed Posting loop and made add-ons billable. **Operator drafting:** `content-draft-service.ts` + admin API `/api/admin/msi/jobs/[jobId]/draft` + `ContentPostDraft` panel on the admin account board (edit caption/graphic/type on the linked content_item, then approve via JobActions → publish). **Add-on billing:** `addon-billing.ts` (env-driven prices `STRIPE_ADDON_PRICE_<ADDON>[_<TIER>]`, gated on `MSI_ADDON_BILLING_ENABLED`, lazy Stripe); `activateAddon`/`deactivateAddon` attach/remove a Stripe subscription item on the org's subscription (best-effort, re-price on tier change), storing `stripeSubscriptionItemId`. Safe no-op until configured. +21 tests; suite **351/351**, type-clean.
- **2026-07-26 (MSI Add-ons: activation stack + Managed Posting workflow).** Made the "MSI as platform, sell add-ons on the same backbone" strategy real. **Activation:** `msi_addon_subscription` table (migration `0054`, one row per (org,addon)), pure catalog `addons.ts` (10 add-ons, discriminated-union pricing, `validateActivation`), `addon-service.ts` (activate/deactivate/list, upsert-on-conflict), `GET/POST /api/msi/addons`, dashboard `/dashboard/infrastructure/addons` (who-performs badges, tier picker). **Managed Posting (first real add-on):** new `content_post` job type (draft→review→publish, SLA 24h), pure `managed-posting.ts` (per-tier monthly quota 12/30/60), `managed-posting-service.ts` (`getPostingStatus`, quota count this month, `requestManagedPost` → draft content_item + content_post job), `GET/POST /api/msi/addons/managed-posting`, request UI. On QA approval a content_post job enqueues a publish_post (operations-service), reusing the entire publish/worker/billing pipeline unchanged. +17 tests; suite **347/347**, type-clean. Docs §19 updated. WEB: `/managed` positioning page + MSI sections on home/pricing shipped separately.
...
- **2026-07-24 (Diagnostics probes for all platforms)** — Completed the diagnostics instrument: the remaining four clients now implement the `diagnose` probe, so "Run diagnostics" works for every platform (was Instagram-only). Read-only, injectable-fetch, never-throwing probes: `facebook-graph.probeFacebookPage` (`GET /{pageId}?fields=id,name` + `/me/permissions` → `pages_manage_posts`), `tiktok-content.probeTikTok` (`GET /v2/user/info/`), `linkedin-posts.probeLinkedIn` (`GET /v2/me`), `youtube-upload.probeYouTube` (`GET /youtube/v3/channels?mine=true`) — each returns reachable / tokenValid / identity (+ permissions for FB). Each client's `diagnose(ctx)` reveals fresh credentials then calls its probe. +8 tests (success + invalid-token per platform). No new publishing behavior — this is validation tooling for the per-platform live-API verification. Gates: **319/319**, new code type-clean. (Pre-existing unrelated `brand-profile/page.tsx` tsc error still outstanding.)
- **2026-07-24 (Credential capture format — JSON for API accounts, validated at capture)** — Resolved the capture-format ambiguity: for `official_api` accounts the vault blob is **JSON** with the platform's fields (the client `JSON.parse`s it), not the plaintext login the placeholder implied. Pure `credential-format.ts` (+8 tests): `REQUIRED_CREDENTIAL_FIELDS` (instagram `accessToken`+`igUserId`, facebook `accessToken`+`pageId`, tiktok `accessToken`, linkedin `accessToken`+`authorUrn`, youtube `accessToken` — mirrors each client's parser), `credentialTemplate(platform)` (editable JSON hint incl. optional fields), `validateCredentialBlob(platform, raw)` (valid JSON + required fields non-empty; extra login fields allowed + preserved for handoff; unknown/manual → freeform). Capture route now loads the account's platform+strategy and **validates official_api blobs up front** → 400 with a clear message (fail at capture, not at publish). `VaultActions` gained `platform`+`apiFormat` props: API accounts show the JSON template placeholder + "required: …" helper; manual accounts keep the freeform login placeholder. Gates: **311/311**, new code type-clean. (Pre-existing unrelated `brand-profile/page.tsx` tsc error still outstanding.)
- **2026-07-24 (Ops diagnostics page — triage before validation)** — A per-account diagnostics surface so a failing account is triaged in one place instead of hunting logs — the highest-leverage next step for the live-API **validation** phase (Instagram first). **Pure core** `diagnostics.ts` (+9 tests): `buildChecks(facts)` composes DB/vault facts + an optional live probe into an ordered check list (credentials captured, execution strategy, token expiry ok/warn/fail/unknown, live API reachable / token valid / identity / per-permission, last publish success+latency / failure+reason) + `overallStatus`. **Live probe seam:** `PlatformClient.diagnose?(ctx)` → `PlatformDiagnosis` (reachable/tokenValid/identity/permissions), surfaced on `ExecutionAdapter.diagnose` (like `checkStatus`). **Instagram probe implemented** (`meta-graph.probeInstagram` — read-only `GET /{igUserId}?fields=id,username` + `/me/permissions`; +2 tests); other platforms show "no live probe wired" until implemented. `diagnostics-service.runAccountDiagnostics` gathers the facts (credential row, vault expiry, resolved strategy, last publish job) + runs the probe best-effort. Endpoint `POST /api/admin/msi/accounts/[id]/diagnose`; `DiagnosticsPanel` (client) on the Ops account page with a "Run diagnostics" button. Gates: **303/303** tests, diagnostics type-clean. **Status framing (corrected):** the execution framework + client implementations are feature-complete *in code*; production readiness now depends on validating each client against the live platform APIs, credentials, permissions, app-review, and edge cases — the diagnostics page is the tool for exactly that. (Note: an unrelated pre-existing tsc error in `dashboard/brand-profile/page.tsx` — unused imports, not MSI — is outstanding from outside this work.)
- **2026-07-24 (LinkedIn video support)** — LinkedIn now publishes single video, alongside text + image. `linkedin-posts.ts`: generalized `registerImageUpload` → `registerUpload(recipe)` and `uploadImageAsset` → `uploadAsset` (both image + video are the same register→PUT-bytes→post flow); added `VIDEO_RECIPE` (`feedshare-video`); `publishToLinkedIn` now takes an optional `videoUrl` → registers the video recipe, uploads the bytes, and posts with `shareMediaCategory: VIDEO` (reuses `buildUgcPostBody`). `linkedin-client.ts`: dropped the "video not supported" guard — `execute` routes `isVideo` → `{ videoUrl }`, else `{ imageUrls }`. +2 tests (VIDEO post body + video publish path asserting the video recipe + category). Video is a **single-PUT** byte upload (synchronous, like the image path) — fine for typical managed clips; **large** videos are the noted follow-up (multipart `/rest/videos`, which could ride the same `execution_handle` async mechanism as YouTube). Gates: `check-types` clean, suite **292/292**. LinkedIn is now feature-complete (text/image/carousel-of-images/video).
